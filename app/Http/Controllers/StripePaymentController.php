...
class StripePaymentController extends Controller
{
    public function store(Request $request,$id){
        ...
        if($stripe){
            ...
            $session  = \Stripe\Checkout\Session::create([
                ...
                'mode' => 'payment',
                'success_url' => $url.'/paymentComplete?session_id={CHECKOUT_SESSION_ID}',
            ...
            return redirect('/dashboard')->with('error','Stripe key not available! Please contact admin');
        }
    ...
    public function payment_complete(Request $request){
        ...
        if($stripe){
            ...
            $pay = $stripe->paymentIntents->retrieve($session->payment_intent);
            ...
            if($pay->status == 'succeeded'){
                ...
                return  redirect('/dashboard')->with('success', 'Your payment has made successfully please check your email.');
            }
            else{
                return redirect('/dashboard')->with('error','Something went wrong! Payment failed');
            }
        }
        else{
            return redirect('/dashboard')->with('error','Stripe key not available! Please contact admin');
