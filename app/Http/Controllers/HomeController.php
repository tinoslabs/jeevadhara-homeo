...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($role == 'admin') {
            ...
            $tot_accountants = $accountant_role->users()->with('roles')->where('is_deleted', 0)->get();
            ...
            }])->whereDate('created_at', Carbon::today())->where('payment_status', 'Paid')->pluck('id');
            // return $invoice;
            ...
            $data = [
                ...
                'total_accountants' => $tot_accountants->count(),
            ...
            $doctor_count = ReceptionListDoctor::where('doctor_id', $doctor_id)->pluck('reception_id');
            $Invoice_Detail = Invoice::withCount(['invoice_detail as total' => function ($re) {
                $re->select(DB::raw('SUM(amount)'));
            }])->where('doctor_id', $doctor_id)->where('payment_status', 'Paid')->pluck('id');

            $revenue = InvoiceDetail::whereIn('invoice_id', $Invoice_Detail)->where('is_deleted', 0)->sum('amount');

            $invoice = Invoice::withCount(['invoice_detail as total' => function ($re) {
                $re->select(DB::raw('SUM(amount)'));
            }])->where('doctor_id', $doctor_id)->where('payment_status', 'Paid')
            ...
            $today_appointment = Appointment::where(function ($re) use ($doctor_id) {
                $re->orWhere('booked_by', $doctor_id);
                $re->orWhere('appointment_with', $doctor_id);
            ...
            $Upcoming_appointment = Appointment::where(function ($re) use ($doctor_id) {
                ...
                ->get();
            $tomorrow_appointment = Appointment::with('patient', 'doctor')->where(function ($re) use ($doctor_id) {
                $re->orWhere('booked_by', $doctor_id);
                $re->orWhere('appointment_with', $doctor_id);
            })->where(function ($re) {
            ...
            $revenue = InvoiceDetail::whereIn('invoice_id', $daily_earning)->where('is_deleted', 0)->sum('amount');
            $invoice = Invoice::withCount(['invoice_detail as total' => function ($re) {
                $re->select(DB::raw('SUM(amount)'));
            ...
            $invoices = Invoice::where('is_deleted', 0)->count();
